\subsection{Performance comparisons}\label{subsec:performance-com}
...
\subsubsection{Experiment with one convolution}\label{sec:experiment-one-convolution}
...
\subsubsection{LGCN PAS performance}
As can be seen from the results in the experiments that we ran, our extension of LightGCN called LGCN PAS performed considerably worse then all of the other methods.
...
Another reason could be that adding the additional information to the embeddings creates noise that disturbs the collaborative signals in the model.
...
Our approach is quite naïve compared to how price-aware recommendation handle the embeddings which is most likely the reason behind why our method did not perform very well.
