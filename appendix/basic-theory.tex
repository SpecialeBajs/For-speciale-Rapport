...
Throughout this subsection we are describing NDCG, recall, training accuracy and test accuracy.
...
Recall and precision is calculated from the possible outcomes of the recommended items, which can be seen in \autoref{tab:possible-results}.
\begin{table}[]
    \centering
    \begin{tabular}{|l|l|l|}
        \hline
...
\end{table}
Precision describes how precise and accurate the model is from the recommended items, so taking $TP$ and $FP$ into account.
Recall describes how accurate the model is related to the items that should be recommended, so taking $TP$ and $FN$ into account.
